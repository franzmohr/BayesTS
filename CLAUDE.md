...
`bayests check` must refuse by name), `cli.interrupted` (a posterior left marked
"writing" is estimated again and refused by later stages) and `agents.recipes`
(see "The agent documentation" below). Each
...
well over 1.5 MB (402 tests, 120 fixtures, from a clean clone), and a recording of it around

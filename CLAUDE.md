...
Twelve of the fifteen samplers are covered from a clean clone.
`VarNormalWishart` and `VecNormalWishart` cannot be generated by
`test/make_model_fixture.cpp` — they can only be read from or derived from a
recorded model file, and `*.h5` is gitignored, so their tests are skipped with a
note at configure time unless pointed at one:

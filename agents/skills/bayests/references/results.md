...
| `<block>/omega` | `(n, iterations)` | `coefficients`, for any time-varying block whose prior sets `omega_v`: the signed standard deviation of the random walk's innovations. `<block>` is `/posterior/a`, `/posterior/psi`, `/posterior/lambda` for the factor models, or `/posterior/u_sigma_inv` and, for `DfmTvpStochvol`, `/posterior/v_sigma_inv` for the stochastic volatility ones, beside that block's `sigma`, which is still written, as `omega^2` |
...
coefficients are not linear in the states, so the change of basis is made again
at every scored period rather than once per draw. It is the same walk the

...
| `<block>/omega` | `(n, iterations)` | `coefficients`, for a `VarTvpStochvol`, `VarTvpGamma` or `VecTvpStochvol` block whose prior sets `omega_v`: the signed standard deviation of the random walk's innovations. `<block>` is `/posterior/a`, `/posterior/psi` or, for the two stochastic volatility models, `/posterior/u_sigma_inv`, beside that block's `sigma`, which is still written, as `omega^2` |

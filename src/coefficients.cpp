...
static int process_single_file_coefficients(const std::filesystem::path &filepath)
{
	try
	{
...
int coefficients(int argc, char *argv[])
{
	...
	if (argc < 3)
	{
		std::cerr << "Usage: bayests coefficients <file.h5 | directory>" << std::endl;
		return 2;
	}
	...
	if (std::filesystem::is_directory(filepath))
	{
		...
		for (const auto &entry : std::filesystem::recursive_directory_iterator(filepath))
		{
			if (entry.is_regular_file() && is_hdf5_file(entry.path()))
			{
				...
				failures += process_single_file_coefficients(entry.path());
			}
		}
	...
	else if (std::filesystem::is_regular_file(filepath))
	{
		...
		return process_single_file_coefficients(filepath);
	}
	else
	{

...
// HDF5 front-end for the VarTvpAld sampler.
//
// The numerics live in src/core/models/var_tvp_ald.cpp and know nothing about
...
namespace
{
namespace io = bayests::hdf5_io::var_tvp_ald;
}
...
void VarTvpAld::draw_coefficients(const ModelLocation &location_arg)
{
    ...
    try
    {
        ...
        const bayests::VarTvpAldInput input = io::read_input(file);
...
void VarTvpAld::forecast(const ModelLocation &location_arg)
{
    // Store the location for later use
    this->location = location_arg;
...
void VarTvpAld::log_likelihood(const ModelLocation &location_arg)
{
    // Store the location for later use
    this->location = location_arg;
    ...
    try
    {
        ...
        const bayests::VarTvpAldInput input = io::read_input(file);
        const bayests::VarTvpAldDraws draws = io::read_coefficients(file);

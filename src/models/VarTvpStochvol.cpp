...
// HDF5 front-end for the VarTvpStochvol sampler.
//
// The numerics live in src/core/models/var_tvp_stochvol.cpp and know nothing
...
#include "io/hdf5/var_tvp_stochvol_io.h"
...
namespace
{
namespace io = bayests::hdf5_io::var_tvp_stochvol;
}

VarTvpStochvol::VarTvpStochvol()
{
}
...
void VarTvpStochvol::draw_coefficients(const std::filesystem::path &filepath_arg)
{
...
void VarTvpStochvol::forecast(const std::filesystem::path &filepath_arg)
{
    // Store filepath for later use
    this->filepath = filepath_arg;
    ...
    try
    {
        ...
        const bayests::VarTvpStochvolDraws draws = io::read_forecast_coefficients(file, input);
...
void VarTvpStochvol::log_likelihood(const std::filesystem::path &filepath_arg)
{
    // Store filepath for later use
    this->filepath = filepath_arg;
    ...
    try
    {
        ...
        const bayests::VarTvpStochvolInput input = io::read_input(file);
        const bayests::VarTvpStochvolDraws draws = io::read_loglik_coefficients(file, input);

        const arma::mat loglik =
            bayests::VarTvpStochvolSampler{}.log_likelihood(input, draws);

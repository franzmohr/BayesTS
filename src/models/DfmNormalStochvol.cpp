...
// HDF5 front-end for the DfmNormalStochvol sampler.
//
// The numerics live in src/core/models/dfm_normal_stochvol.cpp and know nothing
...
namespace
{
namespace io = bayests::hdf5_io::dfm_normal_stochvol;
}

DfmNormalStochvol::DfmNormalStochvol()
{
}
...
void DfmNormalStochvol::draw_coefficients(const ModelLocation &location_arg)
{
...
void DfmNormalStochvol::forecast(const ModelLocation &location_arg)
{
    // Store the location for later use
    this->location = location_arg;
    ...
    // Scored by filtering the realised values through the factors, which is
    // what a model whose history is latent needs -- see factor_score.h.
    if (scorable && !scored)
    {
        const arma::mat score = bayests::DfmNormalStochvolSampler{}.predictive_log_density(input, draws);
        bayests::hdf5_io::write_forecast_loglik(file, score);
    }
}

void DfmNormalStochvol::log_likelihood(const ModelLocation &location_arg)
{
    // Store the location for later use
    this->location = location_arg;
    ...
    const bayests::DfmNormalStochvolInput input = io::read_input(file);
    const bayests::DfmNormalStochvolDraws draws = io::read_coefficients(file);

    const arma::mat loglik =
        bayests::DfmNormalStochvolSampler{}.log_likelihood(input, draws);

    bayests::hdf5_io::write_log_likelihood(file, loglik);
}

ModelCheck DfmNormalStochvol::check(const ModelLocation &location_arg)
{

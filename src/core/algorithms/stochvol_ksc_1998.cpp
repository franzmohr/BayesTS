...
const arma::rowvec::fixed<10> kMixtureWeight = {0.00730, 0.10556, 0.00002, 0.04395,
                                                0.34001, 0.24566, 0.25750};
const arma::rowvec::fixed<10> kMixtureMean = {-11.40039, -5.24321, -9.83726, 1.50746,
                                              -0.65098, 0.52478, -2.35859};
const arma::rowvec::fixed<10> kMixtureVariance = {5.79596, 2.61369, 5.17950, 0.16735,
...
{
  if (!ok) {
    throw std::invalid_argument("stochvol_ksc_1998: " + what);
  }
}
 ...
 * Conditional on a mixture indicator per period the state space is
                            ...
                            const arma::vec &h_init, const arma::vec &constant)
{
  ...
  for (arma::uword i = 0; i < k; i++) {
    ...
    if (!arma::chol(r, post_h_v)) {
      throw std::runtime_error("stochvol_ksc_1998: the posterior precision of the log-volatility "
    ...
    if (!solved) {
      throw std::runtime_error("stochvol_ksc_1998: the posterior of the log-volatility of " +
                               variable + " is singular");
    }

    const arma::vec h_i = mean + noise;
    if (!all_finite(h_i)) {
      throw std::runtime_error("stochvol_ksc_1998: the drawn log-volatility of " + variable +
                               " is not finite");
    }

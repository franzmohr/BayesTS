...
static int process_single_file_loglik(const ModelLocation &location)
{
	try
	{
...
int loglik(int argc, char *argv[])
{
	...
	if (!parse_command_options(argc, argv, "loglik", false, options))
	{
		return 2;
	}
	...
	if (std::filesystem::is_directory(filepath))
	{
		...
		for (const auto &entry : std::filesystem::recursive_directory_iterator(filepath))
		{
			if (entry.is_regular_file() && is_hdf5_file(entry.path()))
			{
				...
				failures += process_single_file_loglik(location);
			}
		}
	...
	else if (std::filesystem::is_regular_file(filepath))
	{
		...
		return process_single_file_loglik(ModelLocation{filepath, options.group});
	}
	else
	{
